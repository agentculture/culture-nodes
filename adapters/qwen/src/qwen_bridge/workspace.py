...
`begin()` immediately before dispatching the `qwen` subprocess and
...
`claude_code_bridge.workspace` and `colleague_bridge.workspace`.
"""
...
def _run_git(repo: str, *args: str) -> subprocess.CompletedProcess[str] | None:
    ...
    try:
        ...
        # claude_bin/qwen_bin/colleague_bin.
...
def measure(handle: WorkspaceHandle) -> dict[str, Any]:
    ...
    exact shape this bridge shares with `claude_code_bridge`/`colleague_bridge`).

    Every field here comes from a `git` subprocess THIS bridge just ran —
    never from qwen's own reported output. `changed_files` is the union
