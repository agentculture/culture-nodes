...
`adapters/colleague/src/colleague_bridge/callbacks.py`. Mirrors
`tests/conformance/reference.go`'s `postEvent` (the kit's own worked example
of a conforming actor): every event carries a stable `event_id` and the NEXT
sequence number, and a non-2xx delivery is retried with the exact same event
— never a fresh id, never a skipped sequence — because that identity is what
makes the control plane's own deduplication meaningful (PRD §13.4: "repeated
...
logger = logging.getLogger("claude_code_bridge.callbacks")
