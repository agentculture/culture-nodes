...
import pytest
from codex_bridge import codex_cli, scope_guard, server, workspace
...
def _fake_session(writes: dict[str, str]):
    ...
    def fake_run_sync(cfg_, instruction, repo_, *, model, sandbox, continuation_ref=None, **kw):
...
def test_instruction_that_merely_names_a_guarded_path_is_accepted(git_bridge, monkeypatch):
    ...
    monkeypatch.setattr(codex_cli, "run_sync", _fake_session({"notes.md": "no CI here\n"}))
...
def test_session_that_modifies_a_guarded_path_is_refused(git_bridge, monkeypatch):
    base, cfg, repo = git_bridge
    monkeypatch.setattr(
        codex_cli, "run_sync", _fake_session({".github/workflows/go.yml": "name: go\n"})
    )
