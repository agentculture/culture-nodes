...
- code review round (#325): the land checkout lease walks every node-run page and matches the actor key (unmeasured waits); lint-all exit 2 is `measurement_incomplete`; the stage watermark no longer gates `pr.merged`/`pr.closed`; the observer node declares its input; LOCK-mode false rows never expire by age and async `credential_spent` callbacks lock the lane; the liveness gate runs before the per-actor gates; the deploy detector reads `preflight.host.liveness`; `deploy_jira` writes `JIRA_CREATE_PROJECTS`; a never-logged-in lane classifies as `not_logged_in` and doctor reports live/unmeasured/dead; doctor's `_lane_liveness_check` classification loop is extracted into `_tally_lanes` (SonarCloud python:S3776, cognitive complexity 16 → under the 15 allowed); the orphan-intake e2e test's four phases move into named helpers (SonarCloud go:S3776, cognitive complexity 32 → 2); the stage-write-back e2e test's five phases move into named helpers too (SonarCloud go:S3776, cognitive complexity 38 → 1); the land-cutover deploy test's four assertion phases move into named harness helpers (SonarCloud go:S3776, cognitive complexity 27 → under the 15 allowed); `nodes run list`'s row formatting moves into `_format_run_list_line` and the text branch stops nesting under `else` (SonarCloud python:S3776, cognitive complexity 19 → under the 15 allowed); `nodes run list` has one exit path instead of three invariant returns (SonarCloud python:S3516); the cleanup node can no longer cancel a run that left the merge-approval node between its read and its POST — `POST /v1alpha1/runs/{id}/cancel` takes an optional `parked_at` precondition re-checked inside the cancel transaction under the same per-run advisory lock the human-decision advance takes, answering 412 without cancelling when the run has advanced, and `cleanup.py` records that run as `left_running`; the hand-turn observer no longer files a second copy of turns already on the item's ledger — `--post` reads the item's existing `hand_turn` records across all its runs and skips the proposals that repeat one (work item + rule + `observed_at` + `evidence_refs`; `what` and `definition_ref` are deliberately out of that identity), so a rerun or a retry after a half-failed batch cannot inflate the confirmed `hand_turns_by_stage` count a delivery summary cites; the lane-recovery recipe names the sequence that actually reopens a locked lane — `codex login` as the **login user** (bootstrap copies out of the login user's home, not the engine account's), `sudo bash lanes/unix-user.sh bootstrap codex` (the lane's one root step, which refuses a non-root caller), a bridge restart for the bridge's per-process latch, and `POST /v1alpha1/actors/{id}/resume` for the control plane's `locked=true`, which no restart clears — and says to read the actor back, because resume clears the lock and nothing else (`.claude/skills/nodes-operator/SKILL.md`, `deploy/prod/README.md`; Qodo High, PR #326); the merge gate's readiness collector is granted on this deployment instead of declaring refs nothing writes — `deploy/prod/lanes/runner-env-write.sh` stamps `PR_UPKEEP_READINESS_SOURCE_URL`/`_SHA256` from the shipped revision's `readiness.py` and grants the five endpoint values empty (the runner resolves refs with `os.LookupEnv`, so an empty grant resolves where an unset one refuses the whole operation by name), `grant-check.sh`'s allowlist names all seven so the first deploy is not refused by its own preflight, and `tests/test_pr_upkeep_readiness.py` diffs the node's `environmentRefs` against the lane — without this every merge decision on the only path through `readiness` reached an approver with no readiness block (Qodo High, PR #326) (Qodo, PR #326); a hand-turn definition can no longer supersede a record of any other kind -- `POST /v1alpha1/hand-turn-definitions` now reads the record named in `supersedes` and refuses (400) anything that is not itself a `hand_turn_definition`, since the ledger only checks that the target exists, is unreplaced and shares the run, so a typo'd id would have quietly dropped a confirmed `hand_turn` (or an evidence or review record) from every projection -- and from the `hand_turns_by_stage` count a delivery summary cites -- with nothing in the ledger edited (Qodo High, PR #326); `open-issue.sh --repo OTHER --disposition ...` no longer writes another repository's issue number into this checkout's triage tables -- the wrapper forwards `--repo` to `scripts/triage-rows.py`, which refuses the pair at `--check-only` time (before the issue is posted) because `docs/triage/open-issues.md` is regenerated from `gh issue list --repo agentculture/culture-nodes` and re-read by `triage-report.py --check` against the same repo, so a foreign number is a row the `triage` lint step can never see open again; a caller that brings its own `--triage-dir` is unaffected (Qodo High, PR #326); the land node states — and now measures — the checkout it runs in: `examples/land/workflow.yaml` named no workspace source at all while `land.py` resolves its repository from `NODES_WORKSPACE` (else the job's cwd), so nothing said that `runner://headspace/land` has to be deployed against culture-land's own checkout, and a runner that handed the job an empty directory would have died on `fatal: not a git repository` inside the fetch. `operation.workspaceRef` cannot carry it — it is a JSON Pointer into the run's own surfaces with no node upstream producing the checkout as an artifact, and a container runner's workspace is a copy it destroys, which cannot hold a branch lease that must outlive the job — so the requirement is stated as a deployment grant beside the others, and `land_gate.py` gained `check_workspace`: a new first `workspace` step that refuses `workspace_not_a_checkout` / `origin_not_configured` by name (exit 2) before the fetch, the same shape it already gives `toolchain_missing` (Qodo High, PR #326); `scripts/triage-rows.py` takes its three writes all-or-nothing, so a half-written disposition can be repaired by retrying the same command -- the two CSV appends and the report regeneration are separate writes, and a failure on the second committed the first, after which the retry `open-issue.sh` prints refuses the number it had already written ("issue #N already has a disposition row") before it can add the missing issue-types.csv row or refresh a stale open-issues.md, stranding a just-opened issue with half its triage and no command that fixes it; the three files are now snapshotted before the first write and any that changed are put back if a later one fails, which extends the script's stated "nothing is written" refusal contract from malformed input to the write itself, and a rollback that cannot itself complete names the files that may hold a partial write instead of claiming a clean tree (Qodo High, PR #326); the land gate refuses an `LAND_GATE_JOB` that `scripts/lint-all.sh` does not have (`lint_job_unknown`, exit 2, before the chain starts) and no longer reads an exit 2 that names nothing as `measurement_incomplete` -- the script answers a typo'd job with the same exit 2 it uses for a step it could not measure, having run no linter at all, so `LAND_GATE_JOB=roott` would have landed a handover nothing linted while the ledger recorded a merely incomplete measurement; "could not measure" is now a claim the script has to support by NAMING the step it could not run (Qodo High, PR #326); the clarify-then-commit gate now reads the briefing of the lane it is actually dispatching to — `OpenPreflight` selects by node run AND actor key, and `clarifyGate` asks it for `dc.ActorRef` — because lane liveness picks the actor per claim: a primary that acknowledged its briefing and then went dead rerouted to its fallback, which found that acknowledgement open on the node run and dispatched on it, i.e. on an authorization composed for another host and answered by another actor, with the gate bypassed by the very routing meant to rescue the run; a rerouted lane is simply unbriefed now and is issued a briefing of its own, which holds the work until that lane answers for itself (Qodo High, PR #326); `PATCH /v1alpha1/runs/{id}` applies its two fields all-or-nothing -- the once-only gh:->Jira `work_item` re-key was written before the category was even decoded, so a body with a good work_item and an unusable category answered 400 having already spent the transition, and the obvious repair (the same body, category fixed) was then refused 409 by the half-applied re-key with no way left to set the category at all; the body is now fully validated before anything is written and both UPDATEs run in one transaction, so a refused PATCH leaves the run exactly as it was and the retry is the repair (Qodo High, PR #326); the merge-gate readiness collector no longer reports an error-shaped HTTP 200 as a clean measurement -- `collect_ci`, `collect_sonar` and `collect_threads` coerced every missing response field into an empty list, a zero total or an empty thread tally, so a body that answered but did not carry the field the question was about reached the approval as `open_issues: 0` / `unresolved: 0` with an EMPTY `failures` list, which is precisely what a mergeable PR looks like; the docstring's promise ("an unreadable source is `null` plus a named failure") only ever covered a refused connection, while a GitHub `{"message": "Not Found"}`, a GraphQL `data: null` (which GitHub answers 200, with no `errors` key, on a partial failure) and a Sonar search with no total all took the coercion path; every derived field is now required of the response (`_require_object`/`_require_list`/`_require_count`, raising `MalformedResponse` into the same `failures` entry a refused connection produces), a `hasNextPage` with no `endCursor` is refused rather than followed back to page one and counted twice, and a non-object body is a named failure instead of the uncaught `AttributeError` that cost the whole block (Qodo High, PR #326); the cleanup node can no longer delete a branch another worker pushed to while it was deciding -- `delete_remote_ref` measured reachability on the commit it fetched and then issued an unconditional `git push --delete`, so any commit landing on that ref in the window between the two reads was deleted having never been tested for reachability, and an unreachable tip is by this node's own rule the only copy of that work; the deletion now carries the tested commit as a lease (`--force-with-lease=<ref>:<sha>`), git compares it against the remote's advertisement inside the push, and a refused lease is recorded as `declined: moved` (naming the commit that WAS measured) rather than a failure or a claimed removal -- the same shape `declined: unreachable` already had; the tested commit is also now read back from the fetch (`fetched_commit`) instead of the earlier `ls-remote` listing, so the reachability test, the recorded commit and the lease all state the same sha (Qodo High, PR #326); an async `credential_spent` callback that lost the race for its work item still locks the lane -- `commitTerminal` called `lockLaneIfCredentialSpent` only after `ResumeWaitingWork` succeeded, so the `engine.ErrStaleClaim` branch returned through `late()` without ever touching `actor_liveness`, and that is the commonest async ordering: the node run's deadline fires, the scheduler reclaims the item and records a `timed_out` attempt, and only then does the bridge report the revoked refresh token -- so exactly the lane whose session is provably gone stayed unlocked and was leased again as soon as the collector's fact aged out, its `fallback_actor` never taken; §13.4's refusal is about the ATTEMPT while a spent credential is a fact about the LANE, which is what the helper's own contract already said ("committed OR refused as late"), so the stale-claim branch now locks before it refuses (Qodo High, PR #326); the cleanup node's closed-PR ticket comment no longer states an outcome only the run can know -- the `stage-cleanup-declined` literal said "deleted nothing: every branch and handover ref it found is unreachable from the default branch", which is a per-ref, run-time decision written into a comment fixed at authoring time, and it is false for the ordinary case of a work item whose EARLIER pull request merged: those refs are reachable from the default branch and are deleted on a close too (deleting them destroys nothing -- that leak is what the node exists to close), so the ticket would have carried a false record of the loose ends, the same false record the graph refuses to post when cleanup FAILS; the comment now states the rule the node applied and points at the derived record for what it did, the README and the workflow's own prose stop inferring reachability from the reason, `test_a_closed_pr_still_deletes_a_ref_an_earlier_merge_landed` pins the behaviour and `test_a_stage_literal_states_no_outcome_only_the_run_can_know` pins the claim -- the node's deletion policy is deliberately unchanged, because declining a ref the default branch already carries protects no copy of anything (Qodo High, PR #326); the two pr-upkeep docs no longer promise that an orphaned PR cannot get a second ticket -- `intake-orphan` creates the ticket BEFORE `stamp-pr` writes the key into the PR body and both run with `maxAttempts: 1`, so a run whose stamp failed leaves a ticket that exists and a PR that still reads as orphaned, and the next fact for that PR takes the `orphan` route again (as does a PR whose `Jira:` line a person edits out); the jira bridge has no search verb, so nothing downstream can detect it, and a person on the board who found two tickets had nothing to read that explained them -- `docs/drive-from-jira.md` now names the cause and the repair, `docs/operations/pr-upkeep-lane.md` names the same hole as an operator turn on a red `stamp-pr`, and `TestTheOrphanIdempotencyLimitIsDocumented` pins both the graph ordering and the docs (Qodo High, PR #326); the claude-code bridge no longer derives a liveness verdict in `LOCK` mode -- `capabilities.liveness_fact` read `claudeAiOauth.expiresAt` whatever the mode and stamped the configured mode onto the answer, so the default (`LOCK`) produced a probe-derived `session_ok=false reason=credential_expired` whose `checked_at` moves on every surface read, and `ActorLiveness.Live` refuses a `LOCK`-mode false at ANY age precisely because a latch never re-measures: one collector write of that fact routed the claude lane to its `fallback_actor` and kept it there until a later collector write replaced it, which a bridge that has gone unreachable (or a collector that has stopped) never produces, even though the credential the fact was derived from may have been refreshed a minute later. `LOCK` now probes nothing here, exactly as it does on codex, and reports `unmeasured` -- this backend hangs no `LivenessState` off a run's output, so it genuinely measures nothing in that mode -- and `CHECK` is this bridge's default where `LOCK` is codex's, because the probe here is a free file read rather than a spent micro-session; `test_lock_mode_probes_nothing_and_says_so` and `test_no_false_fact_this_bridge_emits_is_ever_labelled_lock` pin both halves, and the adapter and deploy docs stop calling the mode an operator preference about how strictly the router should read the lane (Qodo High, PR #326); `examples/land/land.py` is back under the repo-wide 1000-line hard limit (`tests/lint/filelength_test.go`, and `tests/test_land_gate.py`'s copy of it) — the review round's last comment took it to 1005 from a file that had already been squeezed to 999, failing both the `test` and `go` jobs, and a fifth bootstrap sibling is not a cheap relief valve because every module the land bootstrap fetches costs a granted URL/digest pair on every host, so the module docstring's long-form prose moved to `examples/land/README.md` beside it: the lease model (the choice the plan asked to have recorded) is now a section there rather than a paragraph in the code, and the credential, redaction and runner-boundary-identity facts join the README's "what it never does"; the docstring keeps the node's shape, its inputs, its ten steps and its exit codes, and names the limit so the next reader knows why the long form lives next door (CI check failure, PR #326); the docs assertion in `tests/test_pr_upkeep_sweep_work_item.py` that a doc names the second-orphan-ticket case is two assertions instead of one `and`, so a failure names which of `second`/`orphan` the doc is missing (SonarCloud python:S9073, PR #326); the closed-PR deletion assertion in `tests/test_cleanup_node.py` that neither landed ref survives is two assertions instead of one `and`, so a failure names which of the branch or the handover ref the close left behind (SonarCloud python:S9073, PR #326)
