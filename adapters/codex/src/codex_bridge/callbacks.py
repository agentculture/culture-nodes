...
This module is protocol-generic — it carries no codex-specific logic at
all — so it is a verbatim port of `colleague_bridge/callbacks.py`.
...
logger = logging.getLogger("codex_bridge.callbacks")

#: Schemes this module will ever `urlopen` — an actor-protocol callback URL
#: is always an HTTP(S) endpoint the control plane advertised; anything else
#: (e.g. `file:`) is refused before ever reaching urlopen, satisfying
#: bandit's B310 "audit url open for permitted schemes" the honest way
#: (an explicit allowlist) rather than by suppressing the check.
...
class CallbackEmitter:
    ...
    def _post_once(self, body: bytes) -> tuple[int, bool]:
        scheme = urllib.parse.urlsplit(self._url).scheme
        if scheme not in _ALLOWED_CALLBACK_SCHEMES:
            logger.warning("refusing to deliver a callback to a non-http(s) URL scheme %r", scheme)
            return 0, False
        ...
        try:
            # Scheme was already checked against _ALLOWED_CALLBACK_SCHEMES above.
