...
writable_roots` widening, and `claude -p` takes no sandbox flag at all — its
...
event through a DETACHED session plus a flight feed rather than a subprocess
it owns — a success proven on the synchronous path says nothing about the
file-mediated one.
"""
...
from claude_code_bridge import claude_cli, flightfiles, mapping, preserve, server, workspace
...
#: The `type: "result"` record a successful claude session ends with — the
#: one shape both terminal paths here classify as `completed`.
OK_RESULT = {
    "type": "result",
    "subtype": "success",
    "is_error": False,
...
def test_a_handover_dispatch_needs_no_sandbox_widening_here(bridge, monkeypatch):
    ...
    left to a comment: this bridge dispatches `claude -p`, which takes no
...
def _seed_feed_with_success(cfg, invocation_id: str) -> None:
    """Write the terminal `result` record a finished claude session leaves in
    its flight feed. The poller's own file-tailing code reads it, so the
    terminal event this produces is built by production code from a real
    file, not injected past it."""
    path = flightfiles.feed_path(cfg.state_dir, invocation_id)
...
def _run_async(cfg, repo, *, handover: bool):
    invocation_id = "cc_handover_async"
    _seed_feed_with_success(cfg, invocation_id)
    proc = _live_pid()
    ...
    try:
        ...
        AsyncRunner(cfg).start(
            start=claude_cli.BackgroundStart(
                handle_id=invocation_id, pid=proc.pid, log_path=str(repo / "unused.log")
            ),
