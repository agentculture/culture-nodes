...
invocation produced, without re-running `claude -p` a second time.

Protocol-level and backend-agnostic — this module is intentionally
near-identical to `adapters/colleague/src/colleague_bridge/idempotency.py`;
nothing about idempotent replay is claude-specific. The store is one JSON
file per key under `Config.state_dir` — durable across bridge restarts, and
directly `cat`-able during debugging. Filenames are a hash of the key rather
than the key itself so an oddly-shaped key can never become a path-traversal
or "file too long" bug.
"""
