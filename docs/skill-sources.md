...
culture-nodes vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. culture-nodes
...
#   - consumer-identifying prose: `guildmaster` → `culture-nodes` (NOT
...
If a re-sync would lose a culture-nodes adaptation, lift the change
