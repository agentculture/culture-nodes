...
calls `AsyncRunner.start()` directly (it owns the `pi --acp` Popen itself
...
synchronous invocation blocks the server for the duration of one `pi
...
from pi_bridge.idempotency import IdempotencyStore
from pi_bridge.session_registry import SessionRegistry

logger = logging.getLogger("pi_bridge.server")
...
def _resume_session_lost(result: pi_cli.SyncRunResult) -> bool:
...
def decide_async(cfg: Config, *, force_async: bool | None, max_steps: int | None) -> bool:
    ...
    `max_steps` here is a dispatch-timing signal only (pi has no native
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_invocation(self) -> None:  # noqa: C901 - one straight-line validation ladder
        ...
        # `pi_cli.spawn`/`run_sync`, because it was previously read nowhere:
        ...
        if sandbox is not None and sandbox not in pi_cli.SANDBOX_MODES:
            self._write_json(
                400,
                {
                    "error": f"sandbox {sandbox!r} is not one of {sorted(pi_cli.SANDBOX_MODES)}",
                    "class": mapping.CLASS_ACTOR_REJECTED_INPUT,
                },
            )
            return
        ...
        # pi_cli and preserve.handover_ref — were written and unit-tested
        ...
        if handover and sandbox != pi_cli.SANDBOX_WORKSPACE_WRITE:
            self._write_json(
                400,
                {
                    "error": (
        ...
        *,
        ...
        try:
            invocation_id = self.bridge.async_runner.start(
                ...
                # slot once pi's turn actually finishes — None here
        ...
        except pi_cli.SpawnError as exc:
            if held:
                self.bridge.session_registry.release(session_key, idem_key)
...
def _refuse_unauthenticated_exposure(cfg: Config) -> None:
    ...
    (PI_BRIDGE_ALLOW_UNAUTHENTICATED=1) — never by accident."""
    if cfg.auth_token or cfg.host in _LOOPBACK_HOSTS:
        return
    if os.environ.get("PI_BRIDGE_ALLOW_UNAUTHENTICATED") == "1":
        logger.warning(
            "serving on %s WITHOUT authentication " "(PI_BRIDGE_ALLOW_UNAUTHENTICATED=1)",
            cfg.host,
        )
        return
    raise SystemExit(
        f"refusing to bind {cfg.host} without an auth token: set "
        "PI_BRIDGE_AUTH_TOKEN, bind to loopback, or set "
        "PI_BRIDGE_ALLOW_UNAUTHENTICATED=1 to accept an "
...
def serve_forever(cfg: Config) -> None:  # pragma: no cover - exercised via __main__, not unit tests
    ...
    logger.info("pi-bridge listening on http://%s:%d", cfg.host, server.server_address[1])
    try:
