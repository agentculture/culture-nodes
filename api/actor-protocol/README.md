...
| `liveness` | Whether this lane's engine **session** is live (issue #308): `{session_ok, reason, checked_at, mode}`. `session_ok` is `true`, `false`, or `null` for "nobody measured this" — a stale or unmeasured lane is never refused a lease, only a measured-dead one. `reason` is from a closed vocabulary (`ok`, `unmeasured`, `refresh_token_spent`, `credential_expired`, `probe_timeout`, `probe_failed`); `mode` is the lane's operator-configured `LOCK` (latch on the first spent-credential failure, hold until cleared) or `CHECK` (probe before dispatch on the collector's clock). Omitted by a bridge that measures no session credential. Shape and vocabulary live in the shared `liveness.py`. |
