...
def pending(base: str, run_id: str | None = None) -> list[dict]:
    ...
    Falls back to computing the same rule client-side when the control plane
    predates the endpoint. ADR 0002 requires N-1 binary compatibility, and the
    operator's tooling is on the wrong side of that during a rollout: the
    endpoint shipped in this cycle, and the deployed control plane will not
    have it until the next deploy. A tool that simply broke in that window
    would make the exit gate unrunnable exactly while a release is in flight.

    The fallback is NOT silent — it says which rule produced the answer,
    because the server-side rule is the authority and a client-side
    reimplementation of it is the thing t30 set out to remove.
    """
    ...
    try:
        ...
        # /v1alpha1/ path, so an absent endpoint is indistinguishable from an
        # empty one by status code alone — the defect issue #8 records.
        print(
            f"decide-claims: {base} has no /v1alpha1/pending-decisions "
            "(control plane predates it); computing the same rule client-side",
            file=sys.stderr,
        )
        return _pending_client_side(base, run_id)


def _pending_client_side(base: str, run_id: str | None) -> list[dict]:
    """The pre-t30 rule: proposed, and named by no committed review record."""
    if not run_id:
        raise SystemExit(
            "decide-claims: --pending across every run needs the control plane's "
            "endpoint; name a run id instead while the deployed binary predates it"
        )
    ledger = request(f"{base}/v1alpha1/runs/{run_id}/ledger")
    ...
    records = [
        {
            "id": i["id"],
            "record_type": i.get("record_type"),
            "origin_actor_id": i.get("origin_actor_id"),
        }
        for i in items
...
def print_pending(groups: list[dict]) -> int:
    ...
    for group in groups:
        ...
        for record in group.get("records", []):
            print(
                f"    {record['id']}  {record['record_type']}  from {record.get('origin_actor_id', '-')}"
            )
    return 0


def _routes_require_token(base: str) -> bool:
    """Do this control plane's review routes demand the decision bearer?

    They were ungated until task t30 — they wrote human-authority records on
    whoever asked. Refusing to proceed without a token against a binary that
    does not want one would strand the operator mid-rollout, so the demand is
    measured rather than assumed.
    """
    try:
        request(f"{base}/v1alpha1/pending-decisions")
        return True
    except SystemExit:
        return True
    except json.JSONDecodeError:
...
def main() -> int:
    ...
    parser.add_argument(
        "--why", help="why the claim was confirmed or rejected (mandatory, and recorded)"
    )
    ...
    parser.add_argument(
        "--pending", action="store_true", help="list everything awaiting a decision, across runs"
    )
    ...
    if not token and _routes_require_token(base):
        print(
