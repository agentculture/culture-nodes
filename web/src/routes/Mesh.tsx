...
function FlowCultureNode({ data }: NodeProps<MeshFlowNode>) {
  ...
  return <><Handle type="target" position={Position.Left} isConnectable={false} /><CultureNode node={node} selected={data.selected} live={data.item.status === "answering"} pulseCount={data.pulseCount} />{data.item.status === "unknown" ? <p className="status-chip status-chip--unknown" title={data.item.error}>unknown · {data.item.error}</p> : null}<Handle type="source" position={Position.Right} isConnectable={false} /></>;
}
...
export function Mesh() {
  ...
  const { positions } = useElkLayout(layoutGraph);
  ...
  const edges: BuiltInEdge[] = graph.edges.map((edge) => { const runId = edge.source.startsWith("run:") ? edge.source.slice(4) : ""; const hasActorEdge = graph.edges.some((candidate) => candidate.source === edge.source && candidate.relation === "run-actor"); const relevant = edge.relation === "run-actor" || (edge.relation === "run-workflow" && !hasActorEdge); const pulse = relevant ? pulseByRun[runId] ?? 0 : 0; return { id: edge.id, source: edge.source, target: edge.target, type: "smoothstep", label: edge.relation, className: `mesh-edge mesh-edge--${edge.relation}`, animated: pulse > 0 && !reducedMotion, markerEnd: { type: MarkerType.ArrowClosed }, data: { pulse } }; });
  ...
  return <section className="view-rail mesh-view"><header className="mesh-view__head"><div><h1>Mesh</h1><p className="muted">Machines, bridges, people, published workflows and active runs, joined only by committed relationships.</p></div><p id="mesh-connection" className="mesh-connection" data-state={status} role="status"><span className="mesh-connection__dot" aria-hidden="true" />{status === "live" ? "live" : "reconnecting"}</p></header>{error ? <ErrorNotice error={error} /> : null}
    <div id="mesh-canvas" className="mesh-canvas canvas-surface" data-motion={reducedMotion ? "static" : "animated"} tabIndex={0} onKeyDown={onKeyDown} style={{ height: 620 }}>
      <ReactFlow nodes={nodes} edges={edges} nodeTypes={NODE_TYPES} fitView nodesFocusable onNodeMouseEnter={(_, node) => setInspectedId(node.id)} onNodeClick={(_, node) => setInspectedId(node.id)}><Background /></ReactFlow>
    </div>
    <aside id="mesh-tooltip" className="mesh-tooltip" hidden={!inspected} role="status">{inspected ? <><span className="mesh-tooltip__label">{inspected.label}</span><span className="mesh-tooltip__sub">{inspected.kind} · traces to {inspected.trace.surface}: {inspected.trace.row}{inspected.status === "unknown" ? ` · unknown · ${inspected.error}` : ""}</span></> : null}</aside>
