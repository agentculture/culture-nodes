...
import pytest
from codex_bridge import codex_cli, repositories, server
...
def test_repo_identities_loads_from_a_config_file_and_from_the_env(tmp_path):
    ...
    cfg = Config.load(
        str(config_file),
        env={"CODEX_BRIDGE_REPO_IDENTITIES": "daria=/srv/git/daria"},
    )
    assert cfg.repo_identities == {"daria": "/srv/git/daria"}

    with pytest.raises(ConfigError):
        Config.load(env={"CODEX_BRIDGE_REPO_IDENTITIES": "no-equals-sign"})
