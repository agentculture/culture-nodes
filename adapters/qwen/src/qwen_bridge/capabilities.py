...
differs here is only what genuinely differs between backends — for qwen,
the `--sandbox` vocabulary a dispatch may name (which the bridge maps onto
qwen's ACP session mode at dispatch time) and the confinement story that
vocabulary rests on. The measured prose in this file is the ported codex
sibling's baseline: plan task t3 re-measures it on the qwen hosts before
...
#: The `--sandbox` values a dispatch may name, in increasing
#: order of what they permit. Ordered (where `qwen_cli.SANDBOX_MODES` is an
...
#: The two modes whose confinement this backend implements with a bubblewrap helper
...
#:
#: `qwen` itself requires nothing because it is what CREATES the sandbox —
#: it runs as this bridge process does. Its version is here to be watched:
#: the probe findings below are the ported codex baseline, pinned to
#: codex-cli's behaviour, so a re-measurement (plan task t3) changes the
#: recorded baseline and re-opens them (`scripts/
...
# the process's answer. qwen is that bridge. This process is inside no
# sandbox, so it writes under `.git` freely — while a `workspace-write`
# session on the same host cannot, because the backend carves `.git` out of
# its writable roots (`qwen_cli.git_writable_override`; the codex baseline
# measured both halves at commit df7d974 on refs/culture-nodes/probe on
# thor). Reporting `supported` from here would state, in the one field a
# consumer reads to decide whether a handover ref can be created, the
# opposite of what a dispatch gets.
#
...
# the three modes carve `.git` out and `danger-full-access` does not, and
...
#
# Making the attempt needs a non-billable way to run one command under the
# backend's own sandbox. The codex baseline found none it could rely on
# (codex-cli 0.147.0: `codex debug` has only models/app-server/prompt-input,
# and `codex sandbox` refuses without a `[permissions]` table no deployed
# host configures); the qwen equivalent is plan task t3's probe. When
...
def host_facts(
    ...
    *git_probe* is the write attempt behind `git_metadata_writable` (issue
    ...
    itself, running `python3 -m qwen_bridge.preflight --git-metadata <repo>`
...
def _confinement(unavailable: dict[str, str]) -> str:
    ...
    if unavailable:
        return (
            "nothing is confined on this host: the backend enforces --sandbox with a bubblewrap "
    ...
    return (
        "the backend enforces --sandbox with a bubblewrap helper backed by unprivileged user "
