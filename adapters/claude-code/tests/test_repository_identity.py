...
from claude_code_bridge import claude_cli, repositories, server
...
def test_repo_identities_loads_from_a_config_file_and_from_the_env(tmp_path):
    ...
    cfg = Config.load(
        str(config_file),
        env={"CLAUDE_CODE_BRIDGE_REPO_IDENTITIES": "daria=/srv/git/daria"},
    )
    assert cfg.repo_identities == {"daria": "/srv/git/daria"}

    with pytest.raises(ConfigError):
        Config.load(env={"CLAUDE_CODE_BRIDGE_REPO_IDENTITIES": "no-equals-sign"})
