...
The boundary itself is real and stays: GitHub Actions workflow
administration is separately authorized work, excluded from the push
credential the claude-code bridge dispatches under, so an actor here may not
modify `.github/workflows/` as part of an ordinary package. What was wrong
was the enforcement point. The guard this one replaces `grep`ed the
INSTRUCTION TEXT for the path fragment before dispatch, which made it both
over- and under-inclusive at the same time:
...
Mirrored field-for-field by `claude_code_bridge.scope_guard` and
`codex_bridge.scope_guard` (the all-backends rule in CLAUDE.md). The broken
prompt-side guard lived in the claude-code bridge ALONE — a boundary present
in one backend and absent in the other two, which is the all-backends rule's
own definition of a bug. This module is where that boundary starts existing
here.
"""
...
from colleague_bridge import workspace
