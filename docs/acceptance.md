...
**Summary: 41 met, 5 partial, 2 not met.**
...
| Contract and mechanical success-signal validation | **partial** | Contract validation is met: `TestOutputContractViolationIsATechnicalStatus`, `TestUndeclaredOutcomeIsRejected`, `TestLedgerDeltaBeyondTheNodeContractIsRejected`. Mechanical **success-signal** validation is not — see acceptance criterion 14 below. |
...
| 9 | Markdown is a generated reflection of JSON state | **NOT MET** | Nothing in the Go control plane generates Markdown from ledger projections. `nodes explain` and the Python front's `explain` serve a static documentation catalog, which is a different thing. The invariant is not *violated* — JSON is the only authoritative form and no Markdown is derived from it — but the deliverable does not exist. **Unbuilt, not proven.** |
...
| 14 | Required success signals mechanically verify or reject the task | **NOT MET** | What exists: a code node's exit status mechanically decides its domain outcome (proven live, criterion 5), and a `post_run` hook mechanically rejects assurance with a derived record (`TestPostRunRejectAssuranceAppendsDerivedRecord`, honesty condition h32). What does **not** exist: any evaluation of a node's `acceptance.requires` block. `internal/compiler/ledger.go`'s `checkAcceptance` only warns about *unknown* check kinds; nothing evaluates the known ones, and the schema itself says "check payloads stay open until the check registry lands". The reference workflow's `acceptance: [{kind: process_exit, equals: 0}]` is therefore **declarative only**. |
...
- **Acceptance-check registry** (criterion 14) — the single largest missing
  piece of Milestone 1's ledger story.
- **Markdown projection rendering** (criterion 9).
