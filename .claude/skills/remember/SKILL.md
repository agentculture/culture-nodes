...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope culture-nodes
...
scope:** the default personal scope is **private** (`--scope culture-nodes
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `culture-nodes`). Private records are served only to a recall in the same
