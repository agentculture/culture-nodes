...
description: >-
  ...
  record it as a first-class, append-only deviation record via `devague deviate`
  before resuming — never fold a deviation silently into drift after the fact.
  Use when the user says "deviate from the plan", "we need to change the plan
  mid-run", "record a deviation", "this isn't matching the plan anymore", or
  when a task agent discovers the confirmed plan no longer matches reality
  partway through a workforce run. Authored and maintained in
...
<!-- lineage: claude (.claude/skills/deviate/SKILL.md) -> colleague (../colleague/.colleague/skills/deviate.md, ff7331e v1.63.1) -> qwen (this file). Surface adaptation: qwen drives the devague CLI directly through its shell tool; there is no curated tool allow-list to navigate. -->
...
This leg runs through the `devague` CLI — the same agent-agnostic commands
for any agent or operator (if `devague` isn't on `PATH`: `uv tool install
devague`). Qwen Code has no curated tool allow-list: the shell tool
(`run_shell_command`) reaches every `devague deviate` move directly, and the
operator runs the same commands directly in a terminal. `--confirm` and
`--reject` are user-only decisions either way.
...
`dN` record with a reason, an approval, and an optional classification.
The delivery summary quotes these records; it does not reconstruct drift
from memory.
...
`--task` naming the plan item ref is likewise required. Deviation state
persists under `.devague/deliveries/<plan-slug>.json` — a peer of the plan
store, keyed by the plan slug, and never touches the plan JSON itself.
