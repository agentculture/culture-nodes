...
`begin()` immediately before dispatching the `pi` subprocess and
...
def _run_git(repo: str, *args: str) -> subprocess.CompletedProcess[str] | None:
    ...
    try:
        ...
        # claude_bin/pi_bin/colleague_bin.
...
def measure(handle: WorkspaceHandle) -> dict[str, Any]:
    ...
    never from pi's own reported output. `changed_files` is the union
