...
| `internal/repair/liveness.go` | Validator-origin writer (plan loop-closure t10, decision c43): where a dispatch goes when its lane's session cannot start. The routing is a pure function of already-recorded facts — the persisted `actor_liveness` row (`session_ok`, `checked_at`, `locked`), the freshness constant, and the actor's registered `fallback_actor` — so the same row yields the same route every time (§10.4). Not `proposed`: no actor suggested the reroute, a rule applied. Not `confirmed`: a human's lane out of a lock is `POST /v1alpha1/actors/{id}/resume`, which is their own transaction, not this record |
...
| `internal/engine/humantaskexpiry.go` | Engine-origin writer (task t11, spec c6): expiring a pending human task whose subject pull request has already merged. The computation is a pure function of already-recorded facts — the run's own input subject and the delivered `pr.merged` fact — so the same inputs yield the same record every time (§10.4). It is emphatically **not** `confirmed`, and the sharpest case for that in the repo: `DecideHumanTask`'s human review transaction sits in the same package, and an engine that could confirm its own inference would be answering the very question it was asked to ask. Nor `proposed` — nobody suggested this; a rule applied. The record names a registered producer identity (`human_task_expiry`), so an expiry with no accountable producer refuses at append rather than landing anonymously |
