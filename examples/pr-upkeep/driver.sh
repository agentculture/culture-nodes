...
REVIEW_INSTRUCTION="${REVIEW_INSTRUCTION:-Read-only independent review of the fix described in fixReport (see the Bound inputs block for fixReport, fixEvidence, and runEvidence). Analysis only: change nothing. Your FINAL message must be EXACTLY one JSON object of this shape and nothing else: {\"outcome\": \"approve\" or \"changes_required\", \"output\": {\"verdict\": same value as outcome, \"findings\": [{\"title\": string, \"detail\": string}]}} — findings may be empty only with approve.}"
