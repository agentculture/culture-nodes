...
name: challenge
description: >-
  Run a risk-scaled blind-spot discovery pass over a converged, exported frame
  BETWEEN `think` and `spec-to-plan` (the seventh origin skill, third leg in
  flow order): pressure-test the spec through structured lenses, route every
  finding back through the existing deterministic moves as proposed-only content
  the human adjudicates, and on a clean pass record the examined lenses/surfaces
  and residual uncertainty — never a claim that there are no unknown unknowns.
  Use when the user says "challenge this spec", "blind-spot pass", "pressure-
  test the frame", "what are we missing", "unknown unknowns", or after `think`
  exports and before `devague plan new`. Authored and maintained in
...
<!-- lineage: claude (.claude/skills/challenge/SKILL.md) -> colleague (../colleague/.colleague/skills/challenge.md, ff7331e v1.63.1) -> qwen (this file). Surface adaptation: qwen drives the devague CLI directly through its shell tool; there is no curated tool allow-list to navigate. -->
...
The pass drives the `devague` CLI — the same agent-agnostic commands for any
agent or operator (if `devague` isn't on `PATH`: `uv tool install devague`).
Qwen Code has no curated tool allow-list: every move — `capture`,
`interrogate`, `park`, `converge`, `scope`, `question`, `review`, `export`,
and `plan risk` — runs directly through the shell tool
(`run_shell_command`), and the operator runs the same commands directly in a
terminal. `confirm` and `reject` are user-only decisions either way.
...
- **Never conclude there are no unknown unknowns.** Not after a rigorous
  ...
  provenance instead of a comforting absolute (issue 73 success criteria; the
  anti-fabrication contract in `docs/llm-guidance.md`).
