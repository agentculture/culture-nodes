"""TaskResult -> actor-protocol mapping (PRD §13.2/§13.4, qwen session
...
`qwen_cli.parse_session` (not this module) differs in how that
three-way status gets derived from the underlying agent backend.

This module is the one place the bridge decides what a qwen session
(`qwen_cli.parse_session`'s TaskResult-shaped dict) means as an actor
...
unit-testable without ever invoking `qwen`.

The one rule every other decision in this file serves: **incomplete is
never success.** `status == "incomplete"` means qwen's session ended
without ever reaching a terminal session/prompt response — no terminal
response was ever seen, whether because this bridge's own timeout fired,
the process crashed, or it was killed some other way (see `qwen_cli.py`'s
module docstring for the grounding evidence, including the measured case
of a killed session that exited 0 cleanly with no terminal event). This
module reports it as the node's own declared `incomplete` domain outcome
ONLY when the invocation explicitly named one; otherwise it
...
`output` (which carries qwen's own model-claimed `changed_files` — see
...
below. The shape, identical across `claude_code_bridge`, `qwen_bridge`,
...
#: The session-classification vocabulary `qwen_cli.parse_session` produces
...
#: provider-side vocabulary; qwen hands this bridge the same kind of free
#: text — the terminal session/prompt response's error object, rather than
#: a structured error code, so the same text-matching approach applies).
#: Not shared code (each bridge is its own
...
class Classification:
    ...
    #: The delay qwen's own error text named, when `error_class ==
...
def classify(
    ...
    *task_result* is `None` when the qwen subprocess produced no
    parseable output at all (a crash before any transcript line was ever
    written); every other case is what `qwen_cli.parse_session` derived
    from the transcript.
    """
    if task_result is None:
        return Classification(
            domain=False,
            message="qwen produced no parseable result",
    ...
    if status == STATUS_INCOMPLETE:
        ...
        return Classification(
            domain=False,
            message=(
                "qwen's session ended without reaching a terminal session/prompt "
                "response (killed, crashed, or timed out); the invocation declared "
                "no incomplete domain outcome, so this is reported as an execution "
    ...
    if status == STATUS_ERROR:
        ...
        # whether qwen's own error text names a provider capacity
        # refusal — qwen has no separate wire signal for "the provider
        # said no because of quota"; the terminal session/prompt
        # response's error is the only channel
        # (qwen_cli.parse_session captures it into task_result["error"]).
        error_text = str(task_result.get("error") or "")
        if _is_capacity_exhausted(error_text):
            return Classification(
                domain=False,
                message=error_text or "qwen reported a provider capacity refusal",
                error_class=CLASS_CAPACITY_EXHAUSTED,
                retry_after_seconds=_capacity_retry_after_seconds(error_text),
            )
        return Classification(
            domain=False,
            message=task_result.get("error") or "qwen reported a turn failure",
            error_class=CLASS_EXECUTION,
        )

    return Classification(
        domain=False,
        message=f"qwen produced an unrecognised status {status!r}",
        error_class=CLASS_EXECUTION,
    )
...
MODEL_NOT_REPORTED = "unknown:qwen-session-did-not-report"


def usage_from_task_result(task_result: dict[str, Any] | None) -> dict[str, Any] | None:
    """Map provider-reported qwen usage onto §13.2 `Usage`.
    ...
    Qwen does not price its own work here, so `cost`/`currency` stay null.
...
def output_from_task_result(task_result: dict[str, Any] | None) -> dict[str, Any]:
    ...
    artifacts_path}`. qwen has no artifacts-directory convention of its
...
def claim_record(
    ...
    *,
    ...
    qwen's own final message is a completion CLAIM, not verified evidence
    ...
    return {
        ...
        "data": {
            ...
            "qwen_task_id": tr.get("task_id"),
            "qwen_status": tr.get("status"),
        },
        "provenance_refs": [],
        "supersedes": None,
...
def sync_response(
    ...
    *workspace_measured* (task t10) is measured by `workspace.py` around
    ...
    if timed_out:
        body = {
            "error": "qwen did not finish within the bridge's sync timeout",
            "class": CLASS_TIMEOUT,
    ...
    body = {
        ...
        "artifact_refs": [],
        # t5: qwen's own session id (captured by qwen_cli.parse_session
        # from the session/new response) IS the continuation_ref this
        # bridge offers back — the codex seed for this module never
        # touched the backend at all, so its hardcoded None was still
        # standing.
...
def terminal_event(
    ...
    *workspace_measured* (task t10) is attached to EVERY branch below, the
    ...
    if timed_out:
        payload = {
            "class": CLASS_TIMEOUT,
            "message": "qwen did not finish within the bridge's async wait bound",
            "detail": detail,
            "workspace_measured": measured,
        }
