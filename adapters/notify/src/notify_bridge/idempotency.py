...
invocation produced, without posting a second Discord message for the
same attempt.

Protocol-level and backend-agnostic -- this module is intentionally
near-identical to `adapters/human-inbox/src/human_inbox_bridge/
idempotency.py`; nothing about idempotent replay is notify-specific. The
store is one JSON file per key under `Config.state_dir` -- durable across
bridge restarts. Filenames are a hash of the key rather than the key
itself so an oddly-shaped key can never become a path-traversal or "file
too long" bug.
"""
...
class IdempotencyStore:
    ...
    at a time, exactly like the sibling bridges), so the lock here is cheap
    insurance rather than a load-bearing concurrency guard.
    """
