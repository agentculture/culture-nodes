...
| Worker presence rows exist, names only | c19, h19, c38 | **pass with a finding** — . Finding: `hostname` is each worker's container id, not the host (fix package in flight). |
