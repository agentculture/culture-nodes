...
def test_repo_allowlist_from_env_is_pathsep_joined(tmp_path):
    ...
    a.mkdir()
    b.mkdir()
    env = {"COLLEAGUE_BRIDGE_REPO_ALLOWLIST": f"{a}{os.pathsep}{b}"}
...
def test_repo_allowlist_normalizes_relative_and_symlinked_paths(tmp_path):
    ...
    env = {"COLLEAGUE_BRIDGE_REPO_ALLOWLIST": str(link)}
    cfg = Config.load(env=env)
...
def test_config_file_sets_baseline_and_env_overrides_win(tmp_path):
    ...
    env = {"COLLEAGUE_BRIDGE_PORT": "9100"}
...
def test_config_file_via_env_var(tmp_path):
    ...
    env = {"COLLEAGUE_BRIDGE_CONFIG": str(path)}
    cfg = Config.load(env=env)
...
def test_bool_env_parsing(raw, expected):
    cfg = Config.load(env={"COLLEAGUE_BRIDGE_ALWAYS_ASYNC": raw})
    assert cfg.always_async is expected


def test_bool_env_parsing_rejects_garbage():
    with pytest.raises(ConfigError):
        Config.load(env={"COLLEAGUE_BRIDGE_ALWAYS_ASYNC": "maybe"})


def test_int_and_float_env_parsing_reject_garbage():
    with pytest.raises(ConfigError):
        Config.load(env={"COLLEAGUE_BRIDGE_PORT": "not-a-number"})
    with pytest.raises(ConfigError):
        Config.load(env={"COLLEAGUE_BRIDGE_SYNC_TIMEOUT_SECONDS": "not-a-number"})


def test_colleague_env_passthrough_from_file(tmp_path):
    path = tmp_path / "bridge.json"
    path.write_text(json.dumps({"colleague_env": {"COLLEAGUE_ENGINE": "mock"}}))
    cfg = Config.load(str(path), env={})
    assert cfg.colleague_env == {"COLLEAGUE_ENGINE": "mock"}
