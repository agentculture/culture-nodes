...
The bridge runs on the machine that hosts a `colleague` checkout (c15/h13,
c24) — it is a deployment-time reference adapter, not part of the
culture-nodes control plane. Its configuration is deliberately small and
...
* the **colleague binary** to shell out to, and any `COLLEAGUE_*` env
  passthrough (engine/model overrides) the operator wants every dispatch to
  carry;
...
variables (`COLLEAGUE_BRIDGE_*`) override individual fields on top of it, so
an operator can ship one config file per repo checkout and still override a
...
ENV_CONFIG_FILE = "COLLEAGUE_BRIDGE_CONFIG"

#: The `COLLEAGUE_BRIDGE_*` env vars this module recognises, and the config
...
_ENV_STRING_FIELDS = {
    "COLLEAGUE_BRIDGE_COLLEAGUE_BIN": "colleague_bin",
    "COLLEAGUE_BRIDGE_STATE_DIR": "state_dir",
    "COLLEAGUE_BRIDGE_AUTH_TOKEN": "auth_token",
    "COLLEAGUE_BRIDGE_HOST": "host",
    "COLLEAGUE_BRIDGE_DEFAULT_SUCCESS_OUTCOME": "default_success_outcome",
...
_ENV_INT_FIELDS = {
    ...
    "COLLEAGUE_BRIDGE_SYNC_MAX_STEPS": "sync_max_steps",
...
_ENV_FLOAT_FIELDS = {
    ...
    "COLLEAGUE_BRIDGE_CALLBACK_TIMEOUT_SECONDS": "callback_timeout_seconds",
    "COLLEAGUE_BRIDGE_CALLBACK_RETRY_BACKOFF_SECONDS": "callback_retry_backoff_seconds",
...
_ENV_BOOL_FIELDS = {
    ...
    "COLLEAGUE_BRIDGE_PRESERVE_ON_FAILURE": "preserve_on_failure",
    "COLLEAGUE_BRIDGE_PRESERVE_PUSH": "preserve_push",
}

#: `COLLEAGUE_BRIDGE_REPO_ALLOWLIST` is a `os.pathsep`-joined list of
#: absolute repo paths, matching how PATH-shaped env vars are conventionally
#: written; each entry is resolved (symlinks + `..` collapsed) at load time
...
class Config:
    ...
    #: Extra env vars merged onto the subprocess environment (COLLEAGUE_*
    #: engine/model overrides). Operator-supplied; the bridge never invents
    #: values here on its own initiative.
    colleague_env: dict[str, str] = field(default_factory=dict)
    open_pr: bool = False
    ...
    # --- process bounds ----------------------------------------------------
    #: Bounds one SYNCHRONOUS `colleague work` subprocess. On expiry the
    #: bridge sends SIGTERM (never SIGKILL — colleague's own handler commits
    #: partial WIP on SIGTERM) and answers 408.
    sync_timeout_seconds: float = 300.0
    #: Bounds the (normally near-instant) parent `colleague work
    #: --background` call that hands back the start payload.
    background_dispatch_timeout_seconds: float = 30.0
    #: Overall ceiling the async poller waits for a detached colleague run to
    #: produce a result before giving up and reporting a timeout failure.
    #: Generous by default — background work is expected to run long.
    async_wait_seconds: float = 3600.0

    # --- state -------------------------------------------------------------
    state_dir: str = ".colleague-bridge-state"
    ...
    def load(cls, config_path: str | None = None, env: dict[str, str] | None = None) -> "Config":
        ...
        *config_path* wins over `COLLEAGUE_BRIDGE_CONFIG` when both are
        given (an explicit caller argument is the most specific source);
        *env* defaults to `os.environ` and is only ever a parameter so tests
        can supply a clean map instead of mutating the real process
        environment.
        """
...
#: the raw JSON value (so a file-supplied "8085" string integer field still
