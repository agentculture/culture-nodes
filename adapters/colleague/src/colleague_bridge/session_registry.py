...
concurrent same-`session_key` invocation cold rather than queue it, because
this reference bridge's HTTP server is deliberately single-threaded (see
`server.py`) and honest queueing would require decoupling request-accept
from subprocess-execution for the synchronous path, which it does not do.

One backend-specific note: this bridge never actually resumes a session
(`mapping.py` always reports `continuation_ref: null` — issue #62, no
upstream `colleague` resume verb exists yet), so a "fork" here changes no
dispatch behaviour by itself — there is no continuation_ref to discard.
The guard still applies for two reasons: the all-backends rule (t6 is a
concurrency contract across all three bridges, not a resume-specific one),
and because two `colleague work` processes running concurrently against
what the engine considers ONE session_key still race the same live repo
checkout (`workspace.py`'s own "bridges run in the operator's live
checkout" stance) even without a resumable conversation to corrupt. The
fork is always observable — a logged warning, an in-memory `ForkEvent`,
and (per `server.py`) an `X-Session-Fork` response header.
"""
...
logger = logging.getLogger("colleague_bridge.session_registry")
...
class SessionRegistry:
    ...
    def acquire(self, session_key: str | None, holder: str) -> bool:
        ...
        *holder*.
        ...
        Returns True iff *holder* becomes one of the current holders.
        Returns False iff every slot is already occupied (the caller must
        fork) — a `ForkEvent` is recorded and a warning logged before
        returning.
        ...
        with self._lock:
            ...
            if len(current) >= self._max_inflight:
                ...
                logger.warning(
                    ...
                    "(holders=%s); forking invocation %s cold (t6, spec c44/h37)",
                    session_key,
                    len(current),
                    list(current),
                    holder,
