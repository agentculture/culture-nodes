...
`begin()` immediately before dispatching the `claude` subprocess and
...
`codex_bridge.workspace` and `colleague_bridge.workspace`.
"""
...
def measure(handle: WorkspaceHandle) -> dict[str, Any]:
    ...
    exact shape this bridge shares with `codex_bridge`/`colleague_bridge`).

    Every field here comes from a `git` subprocess THIS bridge just ran —
    never from claude's own reported output. `changed_files` is the union
