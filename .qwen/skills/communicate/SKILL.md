...
type: command
description: >
  Cross-repo + mesh communication from culture-nodes: file tracked GitHub issues
  ...
  the next step lives outside culture-nodes (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- culture-nodes (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-culture-nodes
...
<!-- lineage: claude (.claude/skills/communicate/SKILL.md, consumer-adapted per docs/skill-sources.md) -> qwen (this file). Surface adaptation: qwen runs the skill scripts and the devex/agtag CLIs directly via its shell tool, from this skill's base directory; scripts/ re-vendored unmodified. -->
...
Paths in the examples are relative to this skill's base directory (the
`scripts/` alongside this SKILL.md); `agtag` (>=0.1) and `gh` must be on
PATH.

### File a new issue

```bash
bash scripts/post-issue.sh \
...
```bash
bash scripts/fetch-issues.sh 191 --repo agentculture/culture
bash scripts/fetch-issues.sh 191-197 --repo agentculture/culture
bash scripts/fetch-issues.sh 191 195 197
```
