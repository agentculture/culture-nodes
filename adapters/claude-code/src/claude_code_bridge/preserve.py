...
working-tree changes must not evaporate — but the bridge's `claude`
...
Mirrored field-for-field by `codex_bridge.preserve` and
`colleague_bridge.preserve` (all-backends rule): only the docstring's named
subprocess (`claude` here) differs.
...
never on this module's own judgement of what claude reported.
"""
