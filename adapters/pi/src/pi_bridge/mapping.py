"""TaskResult -> actor-protocol mapping (PRD §13.2/§13.4, pi session
...
`pi_cli.parse_session` (not this module) differs in how that
three-way status gets derived from the underlying agent backend.

This module is the one place the bridge decides what a pi session
(`pi_cli.parse_session`'s TaskResult-shaped dict) means as an actor
...
unit-testable without ever invoking `pi`.

The one rule every other decision in this file serves: **incomplete is
never success.** `status == "incomplete"` means pi's session ended
...
the process crashed, or it was killed some other way (see `pi_cli.py`'s
...
`output` (which carries pi's own model-claimed `changed_files` — see
...
below. The shape, identical across `claude_code_bridge`, `pi_bridge`,
...
#: The session-classification vocabulary `pi_cli.parse_session` produces
...
#: provider-side vocabulary; pi hands this bridge the same kind of free
...
class Classification:
    ...
    #: The delay pi's own error text named, when `error_class ==
...
def classify(
    ...
    *task_result* is `None` when the pi subprocess produced no
    parseable output at all (a crash before any transcript line was ever
    written); every other case is what `pi_cli.parse_session` derived
    from the transcript.
    """
    if task_result is None:
        return Classification(
            domain=False,
            message="pi produced no parseable result",
    ...
    if status == STATUS_INCOMPLETE:
        ...
        return Classification(
            domain=False,
            message=(
                "pi's session ended without reaching a terminal session/prompt "
    ...
    if status == STATUS_ERROR:
        ...
        # whether pi's own error text names a provider capacity
        # refusal — pi has no separate wire signal for "the provider
        # said no because of quota"; the terminal session/prompt
        # response's error is the only channel
        # (pi_cli.parse_session captures it into task_result["error"]).
        error_text = str(task_result.get("error") or "")
        if _is_capacity_exhausted(error_text):
            return Classification(
                domain=False,
                message=error_text or "pi reported a provider capacity refusal",
                error_class=CLASS_CAPACITY_EXHAUSTED,
                retry_after_seconds=_capacity_retry_after_seconds(error_text),
            )
        return Classification(
            domain=False,
            message=task_result.get("error") or "pi reported a turn failure",
            error_class=CLASS_EXECUTION,
        )

    return Classification(
        domain=False,
        message=f"pi produced an unrecognised status {status!r}",
        error_class=CLASS_EXECUTION,
    )
...
MODEL_NOT_REPORTED = "unknown:pi-session-did-not-report"


def usage_from_task_result(task_result: dict[str, Any] | None) -> dict[str, Any] | None:
    """Map provider-reported pi usage onto §13.2 `Usage`.
    ...
    Pi does not price its own work here, so `cost`/`currency` stay null.
...
def output_from_task_result(task_result: dict[str, Any] | None) -> dict[str, Any]:
    ...
    artifacts_path}`. pi has no artifacts-directory convention of its
...
def claim_record(
    ...
    *,
    ...
    pi's own final message is a completion CLAIM, not verified evidence
    ...
    return {
        ...
        "data": {
            ...
            "pi_task_id": tr.get("task_id"),
            "pi_status": tr.get("status"),
        },
        "provenance_refs": [],
        "supersedes": None,
...
def sync_response(
    ...
    *workspace_measured* (task t10) is measured by `workspace.py` around
    ...
    if timed_out:
        body = {
            "error": "pi did not finish within the bridge's sync timeout",
            "class": CLASS_TIMEOUT,
    ...
    body = {
        ...
        "artifact_refs": [],
        # t5: pi's own session id (captured by pi_cli.parse_session
...
def terminal_event(
    ...
    *workspace_measured* (task t10) is attached to EVERY branch below, the
    ...
    if timed_out:
        payload = {
            "class": CLASS_TIMEOUT,
            "message": "pi did not finish within the bridge's async wait bound",
            "detail": detail,
            "workspace_measured": measured,
        }
