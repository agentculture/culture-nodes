...
does not itself parent that detached child. `qwen --acp` has no equivalent
detach flag, so this bridge instead spawns and OWNS the `qwen --acp`
...
daemon thread, so the request handler never blocks on qwen finishing.
...
even a qwen session that finishes before the runner thread gets its first
...
from qwen_bridge import mapping, preserve, qwen_cli, scope_guard, workspace
...
logger = logging.getLogger("qwen_bridge.async_runner")
...
class AsyncInvocation:
    ...
    #: invocation's qwen subprocess was spawned; `_run` measures against
    ...
    #: registry to release it from once qwen's turn actually finishes.
...
class AsyncRunner:
    ...
    def start(
        ...
        *,
        ...
        # t9 / #90: threaded through to `qwen_cli.spawn` so an async
        ...
        """Spawn `qwen --acp` in the background and return its invocation
        id immediately. Raises `qwen_cli.SpawnError` if the subprocess
        ...
        spawn-then-register dance), this bridge owns the `qwen --acp`
        ...
        *continuation_ref* (task t5): threaded straight through to
        `qwen_cli.spawn` — the async path is the one long, therefore
        ...
        thread = threading.Thread(
            ...
            name=f"qwen-bridge-run-{invocation_id}",
            daemon=True,
        )
        thread.start()
    ...
    def _run(
        ...
        task_result = qwen_cli.parse_session(stdout_text)
