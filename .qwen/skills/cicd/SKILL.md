...
type: command
description: >
  culture-nodes's CI/CD lane, layered on `devex pr`. Delegates lint / open /
  ...
  unresolved threads). Use when: creating PRs in culture-nodes, handling
...
<!-- lineage: claude (.claude/skills/cicd/SKILL.md, consumer-adapted per docs/skill-sources.md) -> qwen (this file). Surface adaptation: qwen runs the skill scripts and the devex/agtag CLIs directly via its shell tool, from this skill's base directory; scripts/ re-vendored unmodified. -->

# CI/CD — culture-nodes edition
...
`scripts/workflow.sh` is the entry point, resolved from this skill's base
directory (the `scripts/` alongside this SKILL.md). Subcommands:
