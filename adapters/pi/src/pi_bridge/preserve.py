...
working-tree changes must not evaporate — but the bridge's `pi`
...
never on this module's own judgement of what pi reported.
"""
...
def handover_ref(
    ...
    *enabled* is the per-dispatch opt-in. A package that hands nothing over
    ...
    the sandbox side draws: a pi dispatch that hands over no ref is not
