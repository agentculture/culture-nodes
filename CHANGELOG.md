...
- code review round (#325): the land checkout lease walks every node-run page and matches the actor key (unmeasured waits); lint-all exit 2 is `measurement_incomplete`; the stage watermark no longer gates `pr.merged`/`pr.closed`; the observer node declares its input; LOCK-mode false rows never expire by age and async `credential_spent` callbacks lock the lane; the liveness gate runs before the per-actor gates; the deploy detector reads `preflight.host.liveness`; `deploy_jira` writes `JIRA_CREATE_PROJECTS`; a never-logged-in lane classifies as `not_logged_in` and doctor reports live/unmeasured/dead
