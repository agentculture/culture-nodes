...
calls `AsyncRunner.start()` directly (it owns the `qwen --acp` Popen itself
...
synchronous invocation blocks the server for the duration of one `qwen
--acp` call (bounded by `Config.sync_timeout_seconds`); an asynchronous
...
from qwen_bridge.async_runner import AsyncRunner
from qwen_bridge.config import Config
from qwen_bridge.idempotency import IdempotencyStore
from qwen_bridge.session_registry import SessionRegistry

logger = logging.getLogger("qwen_bridge.server")
...
def _resume_session_lost(result: qwen_cli.SyncRunResult) -> bool:
...
def decide_async(cfg: Config, *, force_async: bool | None, max_steps: int | None) -> bool:
    ...
    `max_steps` here is a dispatch-timing signal only (qwen has no native
...
class Handler(BaseHTTPRequestHandler):
    server_version = "qwen-bridge/0.1"
    ...
    def _handle_invocation(self) -> None:  # noqa: C901 - one straight-line validation ladder
        ...
        if sandbox is not None and sandbox not in qwen_cli.SANDBOX_MODES:
            self._write_json(
                400,
                {
                    "error": f"sandbox {sandbox!r} is not one of {sorted(qwen_cli.SANDBOX_MODES)}",
                    "class": mapping.CLASS_ACTOR_REJECTED_INPUT,
                },
            )
            return
        ...
        # qwen_cli and preserve.handover_ref — were written and unit-tested
        ...
        if handover and sandbox != qwen_cli.SANDBOX_WORKSPACE_WRITE:
            self._write_json(
                400,
                {
                    "error": (
    ...
    def _dispatch_sync(
        ...
        *,
        ...
        # to the moment qwen is actually spawned, so head_before/status
        ...
        # A real dispatch happened (qwen was actually invoked) — durably
        # remember the outcome so a redelivered attempt replays it instead
        # of running qwen a second time (PRD §20.3). A pre-dispatch
        ...
        *,
        ...
        try:
            invocation_id = self.bridge.async_runner.start(
                ...
                # slot once qwen's turn actually finishes — None here
        ...
        except qwen_cli.SpawnError as exc:
            if held:
                self.bridge.session_registry.release(session_key, idem_key)
...
def _refuse_unauthenticated_exposure(cfg: Config) -> None:
    ...
    (QWEN_BRIDGE_ALLOW_UNAUTHENTICATED=1) — never by accident."""
    if cfg.auth_token or cfg.host in _LOOPBACK_HOSTS:
        return
    if os.environ.get("QWEN_BRIDGE_ALLOW_UNAUTHENTICATED") == "1":
        logger.warning(
            "serving on %s WITHOUT authentication " "(QWEN_BRIDGE_ALLOW_UNAUTHENTICATED=1)",
            cfg.host,
        )
        return
    raise SystemExit(
        f"refusing to bind {cfg.host} without an auth token: set "
        "QWEN_BRIDGE_AUTH_TOKEN, bind to loopback, or set "
        "QWEN_BRIDGE_ALLOW_UNAUTHENTICATED=1 to accept an "
...
def serve_forever(cfg: Config) -> None:  # pragma: no cover - exercised via __main__, not unit tests
    ...
    logger.info("qwen-bridge listening on http://%s:%d", cfg.host, server.server_address[1])
    try:
