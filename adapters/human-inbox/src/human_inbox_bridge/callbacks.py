...
Protocol-level and backend-agnostic — near-identical to
`adapters/claude-code/src/claude_code_bridge/callbacks.py` and the colleague
original. Mirrors `tests/conformance/reference.go`'s `postEvent` (the kit's
own worked example of a conforming actor): every event carries a stable
`event_id` and the NEXT sequence number, and a non-2xx delivery is retried
with the exact same event — never a fresh id, never a skipped sequence —
because that identity is what makes the control plane's own deduplication
meaningful (PRD §13.4: "repeated callbacks are idempotent").

One human-inbox nuance: because a task may park for days across process
restarts, `server.py` persists the per-task event counter in the task store
and delivers through `resend` with a store-reserved identity, rather than
relying on this object's in-memory counter — `send` remains for callers
whose emitter lives as long as the invocation does.
"""
...
logger = logging.getLogger("human_inbox_bridge.callbacks")
...
class CallbackEmitter:
    ...
    Sequence numbers are assigned by this object (`next_event_id_and_sequence`),
    starting at 1 and incrementing once per DISTINCT event — a redelivery of
    the same event reuses its already-assigned sequence rather than
    consuming a new one, so "monotonically increasing" holds across the
    whole delivered stream, not just the accepted subset.
    """
    ...
    def resend(
        ...
        never risks minting a new id/sequence for what must remain the same
        event.
        """
        body = json.dumps(
    ...
    def _post_once(self, body: bytes) -> tuple[int, bool]:
        ...
        try:
            with urllib.request.urlopen(  # nosec B310 - URL comes from the invocation itself
