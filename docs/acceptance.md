...
| A proposed claim can be decided, and the decision names who decided and why | **met** (task t30, #99) | `GET /v1alpha1/pending-decisions` (`internal/api/decisions.go`) answers what is awaiting a decision as a join, not an authority filter; the web `/decisions` view (`web/src/routes/Decisions.tsx`) records the verdict, reviewer and rationale from a browser; `rationale` is required by `POST /v1alpha1/reviews/{id}/commit` and lands on each review record. `TestDecideAProposedClaimRecordsWhoDecidedAndWhy`, `TestPendingDecisionsListsProposedClaimsUntilTheyAreDecided`, `TestCommitReviewRecordsTheStatedRationale`, `web/src/routes/Decisions.test.tsx`. Proven live against a local control plane: an agent's grade record confirmed from a browser and read back as a human-origin `confirmed` review naming it, with the claim still reading `proposed` |
| The affirmative half cannot be self-served: only a registered human decides | **met** (task t30, #99) | `checkReviewerIsHuman` in `internal/ledger/ledger.go` resolves the named reviewer through `ledger.Tx.ActorKind` and refuses anything but `human` (`reviewer_must_be_human`). This closed a real hole: `CommitReview` stamps human origin from the request's reviewer id, so the producer/authority matrix could not see the lie, and an agent naming itself confirmed its own claim end to end. `TestCommitReviewRefusesANonHumanReviewer` (stub the check and it fails — verified by stubbing it), `TestCommitReviewRefusesAnUnregisteredReviewer`, `TestDecideRefusesAnAgentReviewer`, `TestCommitReviewAllowsAHumanToDecideItsOwnProposal` (the human-task path must keep working) |
