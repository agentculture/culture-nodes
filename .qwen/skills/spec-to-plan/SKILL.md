...
description: >-
  ...
  build plan", or after the `think` skill exports a spec. Authored and
  ...
  vendored from guildmaster like the other skills here.
---

<!-- lineage: claude (.claude/skills/spec-to-plan/SKILL.md) -> colleague (../colleague/.colleague/skills/spec-to-plan.md, ff7331e v1.63.1) -> qwen (this file). Surface adaptation: qwen drives the devague CLI directly through its shell tool; there is no curated tool allow-list to navigate. -->
...
Invoke the `devague` CLI directly through the shell tool
(`run_shell_command`; it is on PATH — if not: `uv tool install devague`).
Plans persist under `.devague/` in the current directory, alongside the frames
they derive from:
...
Two fields do two distinct jobs on every task (shipped: devague#53 t5):

- **`--accept "<criterion>"`** (repeatable) — the **testable contract**: what
  a test suite checks to prove the task done. Write each as something a cheaper
  ...
  constraints ("pre-existing plans load with no error"). A criterion a
  subagent can't be validated against alone is a summary, not a contract.
...
# flips it back to 'proposed' (the user re-confirms):
devague plan instruct t2 "write tests/test_honesty.py covering each honesty condition"
...
devague plan converge                 # gate; resolve any listed gaps (warnings never block export)
devague plan export                   # writes docs/plans/my-feature.md once converged
devague plan waves --json             # scheduling metadata + the per-task subagent brief
```
...
its sibling `think`. It is the *inverse* of the other skills under this repo's
`.qwen/skills/`, which culture-nodes vendors **from** the upstream sources
tracked in `docs/skill-sources.md`. When ready, guildmaster pulls it **from**
devague and broadcasts it to the rest of the
AgentCulture mesh. The `cite, don't import` policy still holds: downstream repos
copy it, they don't symlink or depend on it. See `docs/skill-sources.md`.
