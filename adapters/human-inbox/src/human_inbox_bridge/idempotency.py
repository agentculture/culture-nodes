...
invocation produced, without parking a second inbox task for the same
attempt.

Protocol-level and backend-agnostic — this module is intentionally
near-identical to `adapters/claude-code/src/claude_code_bridge/
idempotency.py`; nothing about idempotent replay is human-specific. The
store is one JSON file per key under `Config.state_dir` — durable across
bridge restarts, and directly `cat`-able during debugging. Filenames are a
hash of the key rather than the key itself so an oddly-shaped key can never
become a path-traversal or "file too long" bug.
"""
...
class IdempotencyStore:
    ...
    against the one genuine concurrency source: the accepted-event delivery
    thread never touches this store, but a future change should not have to
    ...
    def put(
        ...
        same key. Written atomically (temp file + rename) so a crash
        mid-write never leaves a half-written, unparseable replay file
        behind.
        """
        with self._lock:
            path = self._path(key)
