"""CLI entry point: `qwen-bridge` / `python -m qwen_bridge`.

    qwen-bridge [--config PATH] [--host HOST] [--port PORT]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="qwen-bridge", description=__doc__)
    parser.add_argument(
        "--config", default=None, help="Path to a JSON config file (or set QWEN_BRIDGE_CONFIG)."
    )
    ...
    if not (cfg.repo_allowlist or cfg.repo_allowlist_prefixes):
        print(
            "warning: no repo allowlist configured (QWEN_BRIDGE_REPO_ALLOWLIST or "
            "config file 'repo_allowlist') — every invocation will be refused with 403",
            file=sys.stderr,
        )

    dialin.start("QWEN_BRIDGE", cfg.port)
    serve_forever(cfg)
    return 0
