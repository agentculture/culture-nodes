...
`begin()` immediately before dispatching the `codex` subprocess and
...
def measure(handle: WorkspaceHandle) -> dict[str, Any]:
    ...
    never from codex's own reported output. `changed_files` is the union
