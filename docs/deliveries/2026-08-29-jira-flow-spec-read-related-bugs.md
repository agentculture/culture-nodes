...
- Decisions no deviation record covers, captured here directly:
  - four follow-up packages beyond the split plan (WP-H creation-as-first-sighting, WP-I engine-owned page link, WP-J deploy.sh split; WP-B2 is `d2`) — each a defect the live proof or a guard surfaced, each routed to the free codex host with the operator's approval implicit in the goal ("/deviate as needed"); the session count rose from ≈10 declared to ≈16.
...
- **`deploy.sh` over the 1000-line guard** — WP-J (codex-orin) in flight at the time of writing; the PR waits for it.
