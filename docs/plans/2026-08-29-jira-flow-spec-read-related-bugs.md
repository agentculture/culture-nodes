...
### t10 — Reply endpoint: POST /v1alpha1/tickets/{id}/replies (decision-token guarded, required replier, required client-generated `reply_id`) appends a signal event in-process with origin.kind human and the replier named, and enqueues a display-only mirror comment 'via <replier>' through the jira bridge outbox — the fact and its mirror row are written in ONE store transaction and the request is idempotent on `reply_id`; the page-reply fact and the sweep's comment fact validate against one schema
